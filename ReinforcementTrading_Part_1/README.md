...
Only one command is needed. This script will:
1. Download 7 years of BTC data from Binance (cached locally after first run).
2. Train the PPO model for 10,000,000 steps on 2019-2024 data.
...
```bash
source venv/bin/activate && caffeinate -i python train_btc_live.py
```

### File Structure
| File | Purpose |
|---|---|
| `train_btc_live.py` | **Main script** — Train, evaluate, and plot results (All-in-One). |
| `trading_env.py` | Custom Gym environment with SL/TP logic and reward shaping. |
| `indicators.py` | Technical indicator calculation and Binance data fetching. |
| `trade_btc_live.py` | Inference script for predicting next action on live data. |
| `final_validate_model.py` | Optional — Test the model on a specific historical period. |
| `model_btc_best.zip` | The trained model (10M steps, best out-of-sample performance). |
